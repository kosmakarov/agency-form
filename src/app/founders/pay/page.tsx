...
export default function FoundersPayPage() {
  ...
  const handlePayment = async (paymentMethod: 'sbp' | 'bank_card') => {
    ...
    try {
      const response = await fetch('/api/payment/create', {
        ...
        body: JSON.stringify({ name, email, paymentMethod, product: 'founders' }),
      })
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white flex items-center justify-center p-4">
      <div className="bg-[#151515] border border-white/10 rounded-2xl p-6 sm:p-8 max-w-md w-full">
        <h1 className="text-2xl font-semibold mb-2">Сериал Основателя</h1>
        <p className="text-white/40 text-sm mb-2">69 900 ₽ · личное сопровождение</p>
        <p className="text-white/60 text-sm mb-6">
          Лично помогаю внедрить блог в твою жизнь и превратить его в актив.
        </p>
